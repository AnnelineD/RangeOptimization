...
def make_leaf_nodes(start, step, base, l):
  ...
  pat = minimal_seq(pattern(step, start, base))
...
def next_step(start_split, step_split, prev_nodes, base, l):
  ...
  if step_o == 0:
    pat = minimal_seq(pattern(step_split[0], start_split[0], base))
  else:
    pat_full = minimal_seq(pattern(to_number(step_split, base), to_number(start_split, base), base**len(step_split)))
    pat = [to_size(numberToBase(i, base), len(step_split))[0] for i in pat_full]

  lv_prev_it = iter(cycle(prev_nodes))

  for p in pat:
    lv_new.append(Node({p: next(lv_prev_it)}, l)) # kan het hier dan nooit dat een node meermaals moet verbinden???
